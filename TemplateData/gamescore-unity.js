(() => {
    ...
    class e {
        constructor(e) {
            this.gs = e, this.gs.player.on("change", (() => this.trigger("CallPlayerChange"))), this.gs.player.on("sync", (e => {
        ...
        YandexPlayerGet(e) {
            if(this.PlatformType() == "YANDEX")
            {
                console.info("Yandex");
                this.gs.platform.getNativeSDK().getPlayer().then(_player => 
                {
                    _player.getData([e]).then(data => 
                    {
                        if (data) 
                        {
                            console.info(data);
                            this.trigger("CallPlayerYandexGetString", data.key);
                        }
                        else
                        {
                            this.trigger("CallPlayerYandexGetString", "");
                        }
                    }).catch(() => 
                    {
                        this.trigger("CallPlayerYandexGetString", "");
